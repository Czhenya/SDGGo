...
public class Panel : Singleton<Panel>
{
    ...
    public SDGGoGame game = null;        // 游戏实例
    //Timer timer;                       // 计时器对象
    ParamBase param;
    ...
    public void ReqStartGame() {
        ...
        if (CurrentPlayer.Ins.player_num != 2) {
            gameui.text_GameState.text = "还没有对手进入房间！";
            return;
        }
        Debug.Log("请求开始游戏");
    ...
    {
        if (game.SetMove(index, game.color))
        {
            return true;
        }
        else {
    ...
    {
        int huamnplayer = CurrentPlayer.Ins.user.color;
        // 玩家落子成功后电脑下棋
        if (game.color == huamnplayer && game.SetMove(index, huamnplayer))
        {
            return true;
        }
        else
    ...
    {
        if (game.color == CurrentPlayer.Ins.user.color)
        {
            if (game.SetMove(index, game.color))
            {
                // 通知服务器
    ...
    IEnumerator GNUComputerMove()
    {
        ...
        int computerplayer = CurrentPlayer.Ins.opponent.color;
        // 生成AI落子位置
        Point genm = game.GetGenComputerMove(computerplayer);
        genm = game.PointCorrect(genm);
        // 界面落子
        GoUIManager.Ins.setMove(genm, computerplayer);
        GoUIManager.Ins.setRing(genm);
        if (game.color == computerplayer && game.SetMove(genm, computerplayer))
        {
            // 落子成功
            PlayerChange();
            isAICanMove = true;
        ...
        // 刷新棋盘
        GoUIManager.Ins.UpdateAllMoves();
        yield return 0;
    }
    ...
    {
        // 下棋指示切换
        gameui.colorsToMove[game.color].SetActive(false);
        gameui.colorsToMove[game.GetOppenentColor()].SetActive(true);
    ...
    // 获胜者
    string GetWinnerId() {
        float score = game.GetCurrentScore();
        // 贴目
        score += game.komi;
    ...
    void UpdateSore() {
        // 最新成绩结算
        float wscore = game.GetCurrentScore()+game.komi;
        float bscore = -wscore;
